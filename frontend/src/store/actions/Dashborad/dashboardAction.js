...
export const dashboard_index_data_get = () => async (dispatch) => {
    try {
        const { data: { userView, articleCount, categoryCount, tagCount, subAdminCount, userCount } } = await axios.get('http://localhost:5000/rest-api/get-dashboard-index-data', { withCredentials: true });
        dispatch({
